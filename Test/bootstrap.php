...
/**
 * PHPUnit bootstrap for M2Oidc OIDC standalone unit tests.
 *
 ...
 * classes under M2Oidc\OAuth are loaded via Composer's class map.
 */

// Resolve the module root (one dir up from Test/)
$moduleRoot = dirname(__DIR__);

// Register an auto-loader that maps M2Oidc\OAuth\* to files in this module.
spl_autoload_register(static function (string $class) use ($moduleRoot): void {
    if (!str_starts_with($class, 'M2Oidc\\OAuth\\')) {
        return;
    }

    // Convert namespace to relative path:  M2Oidc\OAuth\Foo\Bar -> Foo/Bar.php
    $relative = str_replace('\\', '/', substr($class, strlen('M2Oidc\\OAuth\\')));
